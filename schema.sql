...
CREATE OR REPLACE FUNCTION "public"."create_user"("p_username" "text", "p_password" "text", "p_role" "text" DEFAULT 'operario'::"text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
  ...
  IF NOT (p_role IN ('operario', 'admin', 'superadmin')) THEN
    RAISE EXCEPTION 'Rol inválido. Usa: operario, admin o superadmin';
  END IF;
...
CREATE POLICY "cont_all" ON "public"."contabilidad" USING (true) WITH CHECK (true);
...
CREATE POLICY "inv_all" ON "public"."inventario" USING (true) WITH CHECK (true);
