...
var _usersVersion = 0;
var _localSaveToastTimer = null;
var _QUEUE_OP_TIMEOUT_MS = 12000;
var _showGananciaTotal = true;
var _syncState = {
  status: 'idle',
  message: 'Sin iniciar',
  lastAt: null,
...
var _eyeIconClosed = '<svg viewBox="0 0 24 24"><path d="M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.83l2.92 2.92c1.51-1.26 2.7-2.89 3.43-4.75-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C10.74 7.13 11.35 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.31-.78l3.15 3.15.02-.16c0-1.66-1.34-3-3-3l-.17.01z"/></svg>';
var _eyeIconOpen = '<svg viewBox="0 0 24 24"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>';

function _fmtSyncDate(ts) {
  if (!ts) return '—';
  try { return new Date(ts).toLocaleString('es-CO'); } catch (e) { return '—'; }
}
...
function _updateSyncButton() {
  ...
  if (_syncState.status === 'syncing') { txt = '⏳ Sincronizando'; tone = 'rgba(245, 158, 11, .35)'; }
  else if (_syncState.status === 'offline') { txt = '🔴 Sin conexión'; tone = 'rgba(239, 68, 68, .35)'; }
  else if (_syncState.status === 'ok') { txt = '🟢 Sincronizado'; tone = 'rgba(34, 197, 94, .35)'; }
  else if (_syncState.status === 'warn') { txt = '🟠 pendientes'; tone = 'rgba(234, 179, 8, .35)'; }
  else if (_syncState.status === 'online') { txt = '🔄 En línea'; tone = 'rgba(96, 165, 250, .35)'; }
...
function _setSyncState(status, message, data) {
  _syncState.status = status || _syncState.status;
  _syncState.message = message || _syncState.message;
  if (data) {
    if (typeof data.lastAt !== 'undefined') _syncState.lastAt = data.lastAt;
    if (typeof data.processed !== 'undefined') _syncState.processed = data.processed;
    if (typeof data.failed !== 'undefined') _syncState.failed = data.failed;
    if (typeof data.pending !== 'undefined') _syncState.pending = data.pending;
  }
  _updateSyncButton();
}
...
async function showSyncStatus() {
  ...
  var txt = [
    'Estado: ' + _syncState.message,
    'Última sincronización: ' + _fmtSyncDate(_syncState.lastAt),
    'Procesadas: ' + (_syncState.processed || 0),
    'Fallidas: ' + (_syncState.failed || 0),
    'Pendientes en cola: ' + q.length
  ].join('\n');
  alert(txt);
}

function _updateGananciaBtn() {
  var btn = document.getElementById('gain-toggle-kpi');
  if (!btn) return;
  btn.innerHTML = _showGananciaTotal ? _eyeIconClosed : _eyeIconOpen;
  btn.setAttribute('aria-label', _showGananciaTotal ? 'Ocultar ganancia total' : 'Mostrar ganancia total');
  btn.title = _showGananciaTotal ? 'Ocultar ganancia total' : 'Mostrar ganancia total';
}

function toggleGananciaTotal() {
  _showGananciaTotal = !_showGananciaTotal;
  _updateGananciaBtn();
  renderDash();
}

function fmtGananciaTotal(n) {
  return _showGananciaTotal ? fmt(n) : '••••••';
}
...
function _getActiveSessionUser(op) {
  return currentUser || sessGet('sess_v9') || (op && op.u) || null;
}

function _getActiveSessionRole(op) {
  return currentRole || sessGet('role_v9') || (op && op.ro) || null;
}

function _showLocalSaveToast() {
  var toast = document.getElementById('local-save-toast');
  if (!toast) {
    toast = document.createElement('div');
    toast.id = 'local-save-toast';
    toast.style.cssText = 'position:fixed;bottom:38px;left:50%;transform:translateX(-50%);z-index:10000;' +
      'padding:8px 14px;border-radius:10px;font-size:12px;font-weight:600;' +
      'background:#1f2937;color:#fff;box-shadow:0 6px 22px rgba(0,0,0,.25);opacity:0;transition:opacity .2s';
    document.body.appendChild(toast);
  }
  toast.textContent = 'Guardado localmente — se sincronizara al reconectar';
  toast.style.opacity = '1';
  clearTimeout(_localSaveToastTimer);
  _localSaveToastTimer = setTimeout(function(){
...
function _showDot(online) {
  ...
  if (!dot) {
    ...
    dot.style.cssText = 'position:fixed;bottom:72px;left:50%;transform:translateX(-50%);z-index:9999;' +
      'padding:6px 18px;border-radius:99px;font-size:12px;font-weight:600;' +
      'box-shadow:0 2px 10px rgba(0,0,0,.2);transition:opacity .5s;pointer-events:none;white-space:nowrap';
...
window.addEventListener('online', function(){
  ...
  _showDot(true);
  _setSyncState('online', 'Conexión restaurada. Preparando sincronización…');
...
window.addEventListener('offline', function(){
  ...
  _showDot(false);
  _setSyncState('offline', 'Sin conexión — guardando localmente');
...
async function flushQueue(){
  ...
  try {
    ...
    _setSyncState('syncing', 'Sincronizando operaciones…', { pending: queue.length });

    /* FIX CRÍTICO */
    if (!queue.length){
      _updateBadge();
      _syncQueued = false;
      _setSyncState('ok', 'Sincronización al día', {
        lastAt: Date.now(),
        processed: 0,
        failed: 0,
        pending: 0
    ...
    for (var i = 0; i < queue.length; i++){
      ...
      try {
        ...
        /* Restaurar sesión desde la sesión local actual */
        var activeUser = _getActiveSessionUser(op);
        var activeRole = _getActiveSessionRole(op);
        if (activeUser && activeRole) {
          await withTimeout(_sb.rpc('set_session_user', {
            p_username: activeUser,
            p_role: activeRole
          }), _QUEUE_OP_TIMEOUT_MS);
        }
        ...
        if (op.op === 'insert') {
          ...
          res = await withTimeout(_sb
            .from(tbl)
            .insert(op.data), _QUEUE_OP_TIMEOUT_MS);
          ...
          res = await withTimeout(_sb
            .from(tbl)
            .update(op.data)
            .eq('id', op.id), _QUEUE_OP_TIMEOUT_MS);
    ...
    var queueAfterFlush = failed.length + addedDuringFlush.length;
    ...
    _setSyncState(
      queueAfterFlush > 0 ? 'warn' : 'ok',
      queueAfterFlush > 0 ? 'Sincronización parcial, quedaron pendientes' : 'Sincronización completada',
      {
        lastAt: Date.now(),
        processed: queue.length - failed.length,
        failed: failed.length,
        pending: queueAfterFlush
      }
    );
...
var nowStr = function () { return new Date().toISOString(); };
...
var toDateObj = function (v) {
  if (!v) return null;
  var d = new Date(v);
  if (!isNaN(d.getTime())) return d;
  var m = String(v).match(/(\d{1,2})\/(\d{1,2})\/(\d{4})(?:,\s*(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(a\.\sm\.|p\.\sm\.)?)?/i);
  if (!m) return null;
  var h = m[4] ? +m[4] : 0;
  var min = m[5] ? +m[5] : 0;
  var sec = m[6] ? +m[6] : 0;
  var ampm = m[7] ? m[7].toLowerCase() : '';
  if (ampm.indexOf('p.') !== -1 && h < 12) h += 12;
  if (ampm.indexOf('a.') !== -1 && h === 12) h = 0;
  return new Date(+m[3], +m[2] - 1, +m[1], h, min, sec);
};
var fmtFecha = function (v) {
  var d = toDateObj(v);
  return d ? d.toLocaleString('es-CO') : (v || '—');
};
...
window.addEventListener('beforeunload', function () {
  try {
    localStorage.removeItem('sess_v9');
    localStorage.removeItem('role_v9');
  } catch (e) {}
});
...
/* ── Inventario — CRUD ────────────────────────────────────────── */
async function dbLoadInv(forceOnline) {
  if (forceOnline === undefined) forceOnline = false;
  if (navigator.onLine && (!_syncQueued || forceOnline)) {
...
/* ── Contabilidad — CRUD ──────────────────────────────────────── */
async function dbLoadCont(forceOnline) {
  if (forceOnline === undefined) forceOnline = false;
  if (navigator.onLine && (!_syncQueued || forceOnline)) {
    var d = await _loadContOnline();
    if (d) {
      var q=await _getQueue();
      var delIds=q.filter(function(op){ return op.table==='cont'&&op.op==='delete'; }).map(function(op){ return op.id; });
      d=d.filter(function(r){ return delIds.indexOf(r.id)===-1; });
      q.filter(function(op){ return op.table==='cont'&&op.op==='update'; }).forEach(function(op){
        var idx=d.findIndex(function(r){ return r.id===op.id; });
        if (idx>=0) d[idx]=Object.assign({},d[idx],contFromDb(Object.assign({id:op.id},op.data)));
      });
...
async function dbUpdateCont(item) {
  ...
  if (isPend||isValidId) {
    ...
    else if (isValidId) { await _enqueue({op:'update',table:'cont',id:item.id,data:dbItem}); }
  }
  await _updateBadge();
}
...
async function dbLoadUsers() {
  ...
  (async function () {
    try {
      ...
      var res = await _sb.from('users_safe').select('*');
      if (myVersion !== _usersVersion) return;
      if (res.error) { console.warn('dbLoadUsers bg:', res.error); return; }
      if (!Array.isArray(res.data)) return;

      /* Evitar parpadeo por respuesta vacía transitoria */
      if (!res.data.length && users && users.length) return;

      _cSet('uc', res.data);
      users = res.data;
      var listBody = document.getElementById('ulist');
      if (listBody) renderUList();
    } catch (e) {
      console.warn('dbLoadUsers bg:', e);
    }
  })();
...
async function dbDeleteUser(username) {
  await _sb.rpc('set_session_user', { p_username: currentUser, p_role: currentRole });
  var { error } = await _sb.rpc('delete_user', { p_username: username });
...
async function dbInsertLog(entry) {
  var ingresoTs = entry.ingresoTS || Date.now();
  var { data, error } = await _sb.from('session_log')
    .insert({ usuario: entry.user, ingreso: entry.ingreso, ingreso_ts: ingresoTs, salida: null, salida_ts: null })
    .select('id, ingreso_ts')
    .single();
  if (error) { console.error('Error insertando sesión:', error); return; }
  entry.id = data ? data.id : entry.id;
  entry.ingresoTS = data && data.ingreso_ts ? data.ingreso_ts : ingresoTs;
}
...
function getSorted(rows) {
  ...
  if (invSort === 'bodega') {
    ...
    arr.sort(function(a,b){
      var ai=typeof a.id==='string'?Number.MAX_SAFE_INTEGER:a.id;
      var bi=typeof b.id==='string'?Number.MAX_SAFE_INTEGER:b.id;
      return bi-ai;
    });
  }
  return arr;
...
async function addInventario() {
  ...
  if (!/^\d+$/.test(g)) { alert('La guía debe contener solo números.'); return; }
  if (b && !/^\d+$/.test(b)) { alert('N° Bodega debe contener solo números.'); return; }
  if (p && !/^\d+$/.test(p)) { alert('PIN debe contener solo números.'); return; }
  var yaExiste = invData.some(function (r) {
    return String(r.guia || '').trim().toLowerCase() === g.toLowerCase();
  });
  if (yaExiste) {
    alert('Esta guía ya existe en inventario.');
    return;
  }
...
function renderInv() {
  ...
  tb.innerHTML = rows.map(function (r, i) {
    ...
    return '<tr class="' + rowClass + '">'
      ...
      + '<td style="font-size:11px;color:var(--text2);white-space:nowrap">' + fmtFecha(r.fecha) + '</td>'
...
async function confirmEditInv() {
  ...
  if (!/^\d+$/.test(g)) { errEl.textContent = 'La guía debe contener solo números.'; errEl.style.display = 'block'; return; }
  if (b && !/^\d+$/.test(b)) { errEl.textContent = 'N° Bodega debe contener solo números.'; errEl.style.display = 'block'; return; }
  if (p && !/^\d+$/.test(p)) { errEl.textContent = 'PIN debe contener solo números.'; errEl.style.display = 'block'; return; }
  ...
  dbUpdateInv(r)
    .catch(function (e) { console.warn('dbUpdateInv:', e); });
  logAction('edicion_inv', r.guia,
    'Editó guía: "' + old.guia + '" → "' + r.guia + '" | Bodega: "' + old.bodega + '" → "' + r.bodega + '" | PIN: "' + old.pin + '" → "' + r.pin + '"')
    .catch(function (e) { console.warn('logAction edicion_inv:', e); });
}
...
async function addContabilidad() {
  ...
  var item = { id: null, fecha: new Date(fecha).toISOString(), equipo: equipo, valorM: m, valorB: b, total: m + b, denoms: denoms };
...
function renderCont() {
  ...
  if (!rows.length) { tb.innerHTML = '<tr><td colspan="7" class="empty">Sin registros</td></tr>'; return; }
  tb.innerHTML = rows.map(function (r, i) {
    var _qc = typeof r.id==='string' ? "'"+r.id+"'" : r.id;
    var del     = isAdmin() ? '<button class="btn-del" onclick="delCont('+_qc+')">✕</button>' : '';
    var editBtn = isAdmin() ? '<button class="btn-ghost" style="padding:3px 9px;font-size:12px" onclick="openEditCont('+_qc+')">✎</button>' : '';
...
function openEditCont(id) {
  if (!isAdmin()) return;
  var r = contData.find(function(x){ return x.id===id||x.id===+id; });
  if (!r) return;
  _editContId = r.id;
  var d = toDateObj(r.fecha) || new Date();
  document.getElementById('ec-fecha').value  = new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
...
async function confirmEditCont() {
  ...
  r.fecha  = new Date(fecha).toISOString();
  r.equipo = equipo; r.valorM = m; r.valorB = b; r.total = m + b; r.denoms = denoms;
  closeEditCont(); renderCont();
  dbUpdateCont(r)
    .catch(function (e) { console.warn('dbUpdateCont:', e); });
  logAction('edicion_cont', r.equipo,
    'Editó registro contable "' + r.equipo + '" | Total: ' + fmt(oldTotal) + ' → ' + fmt(r.total))
    .catch(function (e) { console.warn('logAction edicion_cont:', e); });
}
...
function renderDash() {
  ...
  document.getElementById('dash-kpis').innerHTML =
    ...
    '<div class="kpi">' +
      ...
      '<div class="kpi-value" style="font-size:18px;color:var(--accent)">' + fmtGananciaTotal(tC) + '</div>' +
      '<div class="kpi-sub">' + contData.length + ' registros contables</div>' +
      '<div style="display:flex;justify-content:center;margin-top:6px">' +
        '<button id="gain-toggle-kpi" type="button" class="pw-eye" onclick="toggleGananciaTotal()" style="position:static;transform:none;"></button>' +
      '</div>' +
    '</div>';
  ...
  document.getElementById('dash-cont').innerHTML = lc.length ? lc.map(function (r) {
    return '<div class="mini-row"><div style="font-size:12px"><span style="font-weight:600">' + r.equipo + '</span><br>' +
      '<span style="color:var(--text2)">' + fmtFecha(r.fecha) + '</span></div>' +
      '<div class="mini-val">' + fmtGananciaTotal(r.total) + '</div></div>';
  }).join('') : '<div class="dash-empty">Sin registros</div>';
...
async function changeRole(username, newRole) {
  ...
  var oldRole = u.role;
  u.role = newRole;
  var roleErr = await dbUpdateUserMeta(username, { role: newRole });
  if (roleErr) {
    u.role = oldRole;
    renderUList();
    alert('Error al actualizar rol: ' + (roleErr.message || roleErr));
    return;
  }
...
function renderLog() {
  ...
  tb.innerHTML = rows.map(function (r) {
    ...
    return '<tr>'
      ...
      + '<td style="font-size:12px">' + fmtFecha(r.ingreso) + '</td>'
...
function renderAcciones() {
  ...
  tb.innerHTML = rows.map(function (r) {
    ...
    return '<tr><td>' + tag + '</td>'
      ...
      + '<td style="font-size:12px;color:var(--text3)">' + fmtFecha(r.fecha) + '</td></tr>';
  }).join('');
}
...
function exportExcel(e) {
  ...
  btnDescargar.onclick = function () { doExportExcel(btnDescargar); };
...
function doExportExcel(btn) {
  ...
  try {
    ...
    var invRows = invFiltrado.map(function (r, i) {
      return { '#': i + 1, 'Guia': r.guia, 'N Bodega': r.bodega, 'PIN': r.pin, 'Estado': r.estado || 'pendiente', 'Fecha': fmtFecha(r.fecha) };
    });
    ...
    var contRows = contFiltrado.map(function (r, i) {
      return { '#': i + 1, 'Fecha Hora': fmtFecha(r.fecha), 'Equipo': r.equipo, 'Valor Moneda': r.valorM, 'Valor Billete': r.valorB, 'Total': r.total };
    });
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = originalTxt || 'Descargar';
      btn.style.opacity = '';
