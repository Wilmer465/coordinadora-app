...
/* ── Auto-agregar guía al escanear código de barras (12 dígitos) ── */
var _scanTimer = null;
function autoAddGuia(input) {
  ...
  /* Debounce 150 ms — por si el escáner no envía todos a la vez */
  clearTimeout(_scanTimer);
  if (!val) return;
  _scanTimer = setTimeout(function () {
    if (input.value.trim() === val && val.length === 12) {
      addInventario();
    }
  }, 150);
}
