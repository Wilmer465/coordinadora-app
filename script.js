...
async function toggleEstado(id) {
  /* Buscar por ID string (pend_*) o numérico */
...
function estadoLabel(e, id) {
  /* Poner comillas si el ID es string (pend_*) para que el onclick sea válido */
  var idStr = typeof id === 'string' ? "'" + id + "'" : id;
  if (e === 'entregado')    return '<button class="estado-btn estado-entregado"    onclick="toggleEstado(' + idStr + ')">✓ Entregado</button>';
  if (e === 'no_entregado') return '<button class="estado-btn estado-no-entregado" onclick="toggleEstado(' + idStr + ')">✕ No entregado</button>';
  return '<button class="estado-btn estado-pendiente" onclick="toggleEstado(' + idStr + ')">○ Pendiente</button>';
}
...
function renderInv() {
  ...
  tb.innerHTML = rows.map(function (r, i) {
    ...
    /* Pasar id directamente — estadoLabel ya maneja string vs número */
